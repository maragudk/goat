...
func TestGoogleClient_Prompt(t *testing.T) {
	_ = env.Load("../.env.test.local")

	token := env.GetStringOrDefault("GOOGLE_TOKEN", "123")
	...
	tests := []struct {
		...
		{
			Model: llm.ModelGemini_1_5_Flash,
			Token: token,
		},
	}
	...
	for _, test := range tests {
		t.Run(test.Model.String(), func(t *testing.T) {
			c := llm.NewGoogleClient(llm.NewGoogleClientOptions{
				Model: test.Model,
				Token: test.Token,
			})
